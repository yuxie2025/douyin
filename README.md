# 抖音无水印下载工具(20241220,测试可以使用) 抖音（无水印）最新去水印下载方案
...
## 2.使用webView打开抖音分享链接,setWebViewClient->shouldInterceptRequest拦截无水印视频链接(新版下载,需要获取拦截到的header)

```
#链接会过期，请实时获取(新版链接浏览器不能直接打开，缺少header信息)
